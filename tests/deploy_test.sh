...
# echo "Would you like to install the new version of gl-query ( $NEW_VERSION )? ( YES or NO ) "
# read input
# if [[ $input -eq "YES" ]] || [[ $input -eq "yes" ]] || [[ $input -eq "Yes" ]]
# then
#   pip3 uninstall gl-query
#   pip3 install -i https://test.pypi.org/simple/ gl-query==$NEW_VERSION || true
#   pip3 install -i https://test.pypi.org/simple/ gl-query==$NEW_VERSION || true
# fi
